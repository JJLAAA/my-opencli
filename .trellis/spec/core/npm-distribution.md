...
- AI agents must stop at preparing and verifying the version iteration. Do not run publish commands; the human maintainer manually runs the GitHub Actions publish workflow.
...
| Publish handoff | Agent reports the prepared version and verification status; human maintainer triggers GitHub Actions publish manually |
...
- Good: the agent prepares the version bump and dry-run verification, then leaves the actual GitHub Actions publish run to the human maintainer.
...
- Bad: the agent runs `bun run publish:npm`, `npm publish`, or triggers the GitHub Actions publish workflow unless the human explicitly asks for that publishing action.

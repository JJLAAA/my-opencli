...
## Acceptance Criteria
- [ ] Running `bun run bin/cli.js --help` prints global usage and available commands
- [ ] Running `bun run bin/cli.js help <site>` prints the commands under that site
- [ ] Running `bun run bin/cli.js help <site> <command>` prints concrete usage for that adapter command
- [ ] Running `bun run bin/cli.js <site> <command>` still executes the adapter pipeline

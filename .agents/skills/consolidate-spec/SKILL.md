...
description: Analyze and consolidate this project's `.trellis/spec/` documentation without inventing new rules. Use when Codex is asked to整理/维护/合并/重构/审查 Trellis spec files, detect duplicate or conflicting coding guidelines, identify outdated conventions, improve layer `index.md` Pre-Development Checklists, check `guides/` vs `spec/` content boundaries, or evaluate spec file boundaries for context efficiency using real `.trellis/tasks/**/prd.md` task history. Trigger for requests like "consolidate spec", "整理 spec", "清理 .trellis/spec", "检查 spec 冲突/重复", "优化 pre-development checklist", or "review Trellis spec structure".
---
...
- Resolve conflicts: do not leave contradictory guidance.
- Flag, do not delete: if unsure whether a rule is outdated, mark it for review.
...
2. Look for evidence in recent code via search/read.
3. If both seem valid, flag the conflict and ask the user to decide.
4. Do not guess. Preserve both temporarily if uncertain.

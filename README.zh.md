...
安装一个轻量 wrapper 包，npm 会自动拉取当前 OS/CPU 对应的预编译二进制文件，无需 Bun 或构建步骤。
...
**安装范围：** 安装 TAP 二进制文件只会安装 CLI runtime：命令解析、pipeline 执行、schema 输出、浏览器控制和本地配置。TAP 不会默认捆绑具体站点的适配器，也不会自动把 assistant 指令安装进你的 AI 工具。

只有当你想直接获得某个数据源的现成命令时，才需要安装适配器包。适配器包会把文件写入 `~/.tap/adapters/`，也就是把通用的 `tap <site> <command>` 变成具体数据 pipeline 的那一层。

只有当你希望 AI coding assistant 帮你编写新适配器时，才需要安装 assistant skill。运行已有适配器不需要 skill；skill 的作用是让 assistant 学会 TAP 的适配器编写流程，并把生成的适配器写入你的本地适配器目录。
...
按你使用的 assistant 显式安装 skill。这个命令会把 TAP 的适配器编写指令复制到对应 assistant 的 skills 目录，让 assistant 能按同一套流程完成侦察、schema 设计、实现和验证：
